...
/**
 * A Base64 Encoder/Decoder. $Id: Base64Coder.java,v 1.3 2010/07/28 22:05:30 dwd
 * Exp $
 * 
 * <p>
...
public class FrontierBase64Coder {
	 ...
	 * @return A byte array with the Base64 encoded data.
	 */
	public static byte[] encode(final byte[] in, final int iLen) {
	 ...
	 *            number of bytes to process in <code>in</code>.
	 * @param out
	 *            a byte array with at least ((iLen+2)/3)*4 bytes left
	 * @param op
	 *            starting offset in <code>out</code>
	 * @return number of bytes encoded into <code>out</code> (including starting
	 *         amount)
	 */
	public static int encode(final byte[] in, int ip, final int iLen, final byte[] out,
			int op) {
		final int oDataLen = op + (iLen * 4 + 2) / 3; // output length without
														// padding
		final int ep = ip + iLen;
		while (ip < ep) {
			final int i0 = in[ip++] & 0xff;
			final int i1 = ip < ep ? in[ip++] & 0xff : 0;
			final int i2 = ip < ep ? in[ip++] & 0xff : 0;
			final int o0 = i0 >>> 2;
	...
	public static byte[] decode(final byte[] in) {
		...
		final int oLen = (iLen * 3 + 2) / 4;
		final byte[] out = new byte[oLen];
		int ip = 0;
		int op = 0;
		while (ip < iLen) {
			final int i0 = in[ip++];
			final int i1 = ip < iLen ? in[ip++] : 'A'; // 'A' translates to zero
			final int i2 = ip < iLen ? in[ip++] : 'A';
			final int i3 = ip < iLen ? in[ip++] : 'A';
			if (i0 > 127 || i1 > 127 || i2 > 127 || i3 > 127) {
				throw new IllegalArgumentException(
						"Illegal character in Base64 encoded data i0 = " + i0 + ", i1 = "
								+ i1 + ", i2 = " + i2 + ", i3 = " + i3);
			}
